    ...
    <div className={`min-h-screen ${bgLight}`}>
      ...
      {campaign.status === 'cancelled' && (
        <div className="bg-rose-50 border-b border-rose-200 px-6 py-3">
          <div className="max-w-lg mx-auto text-sm text-rose-800 font-semibold text-center">
            この企画は中止されました。応援いただいた金額は全額返金されます。
          </div>
        </div>
      )}
      ...
      <div className="max-w-lg mx-auto px-6 mt-4">
        <button
          onClick={() =>
            campaign.status === 'cancelled' ? undefined : setShowModal(true)
          }
          disabled={campaign.status === 'cancelled'}
          className={`w-full py-4 rounded-2xl bg-gradient-to-r ${gradientFrom} ${gradientTo} text-white font-bold text-lg shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed`}
        >
